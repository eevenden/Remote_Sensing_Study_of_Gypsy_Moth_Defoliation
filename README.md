...
Perhaps more noteably, the spatial distribution of the classification result differ when one examines the maps. For example, it would be interesting further explore why the "Logistic Regression - All" model produces noticeable spatial clusters of False Alarms & Misses, but the same pattern isn't apparent in the outcomes from the "Discriminant Ananlysis - All" model. 
